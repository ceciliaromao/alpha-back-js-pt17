...
async function getStatesList() {
  ...
  try {
    const res = await getStateData(states);
    for (let i = 0; i < res.length; i++) {
      stateSelected.innerHTML += `<option value="${res[i].id}" >${res[i].nome}</option>`
    }
  } catch (err) {
    msg.textContent = err;
  };
...
async function getCitiesList() {
  ...
  try {
    const res = await getCityData(cities);
    for (let i = 0; i < res.length; i++) {
      citySelected.innerHTML += `<option value="${res[i].id}" >${res[i].nome}</option>`
    }
  } catch (err) {
    msg.textContent = err;
  };
...
async function getWeatherTable() {
  ...
  try {
    const res = await getWeatherData(weather)
    for (let i = 0; i < res.length; i++) {
